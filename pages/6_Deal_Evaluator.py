...
from utils.data_loader import (
    ...
    display_name, SWF_DIMENSIONS,
)
from utils.styles import (
    inject_css, kpi_card, section_header, metric_highlight,
    verdict_badge, format_currency, format_pct, COLORS, PLOTLY_COLORS,
)
...
# ═════════════════════════════════════════════════════════════════════════════
st.markdown(section_header("A. Venture Input"), unsafe_allow_html=True)
...
if input_mode == "Select from Dataset":
    ...
    with col_info1:
        st.markdown(kpi_card("Company", selected_company), unsafe_allow_html=True)
    with col_info2:
        st.markdown(kpi_card("Sector", str(row.get('sector', 'N/A'))), unsafe_allow_html=True)
    with col_info3:
        st.markdown(kpi_card("Funding Stage", str(row.get('funding_stage', 'N/A'))), unsafe_allow_html=True)
    with col_info4:
        st.markdown(kpi_card("Country", str(row.get('country', 'N/A'))), unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════════════════════
st.markdown(section_header("B. Intelligence Profile"), unsafe_allow_html=True)
...
with col_cluster:
    ...
    st.markdown(kpi_card("Assigned Cluster", f"Cluster {assigned_cluster}"), unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════════════════════
st.markdown(section_header("C. Prediction — Multi-Model Consensus"), unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════════════════════
st.markdown(section_header("D. Similar Ventures — Nearest Neighbors"), unsafe_allow_html=True)
