...
st.markdown(section_header("A. The Question — Dependent & Independent Variables"), unsafe_allow_html=True)
...
# =====================================================================
st.markdown("---")
st.markdown(section_header("B. Evidence: OLS Linear Regression"), unsafe_allow_html=True)

# KPI row
k1, k2, k3, k4 = st.columns(4)
with k1:
    st.markdown(kpi_card("R-Squared", f"{ols_stats['r2']:.4f}"), unsafe_allow_html=True)
with k2:
    st.markdown(kpi_card("Adjusted R²", f"{ols_stats['adj_r2']:.4f}"), unsafe_allow_html=True)
with k3:
    st.markdown(kpi_card("F-Statistic", f"{ols_stats['f_stat']:.2f}"), unsafe_allow_html=True)
with k4:
    f_pval = ols_stats['f_pvalue']
    pval_str = f"{f_pval:.2e}" if f_pval < 0.001 else f"{f_pval:.4f}"
    st.markdown(kpi_card("F p-value", pval_str), unsafe_allow_html=True)
...
# =====================================================================
st.markdown("---")
st.markdown(section_header("C. Evidence: Regularization — Ridge / Lasso / ElasticNet"), unsafe_allow_html=True)
...
# =====================================================================
st.markdown("---")
st.markdown(section_header("D. Key Finding & Bridge to Prediction"), unsafe_allow_html=True)
