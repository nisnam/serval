...
from utils.data_loader import load_data, load_arima_results, load_prep_artifacts, load_interpretations, load_frameworks, display_name
from utils.styles import inject_css, kpi_card, section_header, metric_highlight, evidence_panel, format_currency, format_pct, COLORS, PLOTLY_COLORS
...
k1.markdown(kpi_card("Observations", f"{n_rows:,}"), unsafe_allow_html=True)
k2.markdown(kpi_card("Features", f"{n_cols}"), unsafe_allow_html=True)
k3.markdown(kpi_card("Missing Data", f"{missing_pct:.1%}"), unsafe_allow_html=True)
k4.markdown(kpi_card("Base Success Rate", f"{success_rate:.1%}"), unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════════════════════
st.markdown(section_header("A — Data Quality & Feature Engineering"), unsafe_allow_html=True)

with st.expander("Expand: Data Preparation Details", expanded=False):
    st.markdown(evidence_panel("Data Preparation & Feature Engineering"), unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════════════════════
st.markdown(section_header("B — Portfolio Distribution Analysis"), unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════════════════════
st.markdown(section_header("C — Sector x Funding Stage Success Analysis"), unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════════════════════
st.markdown(section_header("D — Correlation Intelligence"), unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════════════════════
st.markdown(section_header("E — Deal Flow Forecasting (ARIMA)"), unsafe_allow_html=True)
...
if arima:
    ...
    if hist.get('values'):
        ...
        st.markdown("**Augmented Dickey-Fuller (ADF) Stationarity Test**")
        ts_values = hist['values']
        try:
            adf_result = adfuller(ts_values, autolag='AIC')
            adf_stat, adf_p, adf_lags, adf_nobs = adf_result[0], adf_result[1], adf_result[2], adf_result[3]
            adf_crit = adf_result[4]

            adf1, adf2, adf3, adf4 = st.columns(4)
            adf1.markdown(kpi_card("ADF Statistic", f"{adf_stat:.3f}"), unsafe_allow_html=True)
            adf2.markdown(kpi_card("p-value", f"{adf_p:.4f}"), unsafe_allow_html=True)
            adf3.markdown(kpi_card("Lags Used", str(adf_lags)), unsafe_allow_html=True)
            adf4.markdown(kpi_card("Observations", str(adf_nobs)), unsafe_allow_html=True)

            crit_text = " | ".join([f"{k}: {v:.3f}" for k, v in adf_crit.items()])
            if adf_p < 0.05:
                st.success(f"**Stationary** (p={adf_p:.4f} < 0.05). The null hypothesis of a unit root is rejected. Critical values: {crit_text}")
            else:
                st.warning(f"**Non-stationary** (p={adf_p:.4f} >= 0.05). The series requires differencing (d=1 in ARIMA). Critical values: {crit_text}")
            st.caption("The ADF test checks whether the time series has a unit root (non-stationary). ARIMA's differencing parameter (d=1) addresses non-stationarity.")
        except Exception:
            st.info("ADF test could not be computed on the available time series.")
    ...
    if model_summary:
        ...
        ms1.markdown(kpi_card("Model Order", str(order)), unsafe_allow_html=True)
        ms2.markdown(kpi_card("AIC", f"{aic:.1f}" if isinstance(aic, (int, float)) else str(aic)), unsafe_allow_html=True)
        ms3.markdown(kpi_card("BIC", f"{bic:.1f}" if isinstance(bic, (int, float)) else str(bic)), unsafe_allow_html=True)
