...
from utils.data_loader import load_data, load_sentiment_results
from utils.styles import (
    inject_css, section_header, metric_highlight,
    kpi_card, COLORS, PLOTLY_COLORS
)
...
# ================================================================
st.markdown(section_header("Methodology: Text Generation & Analysis Pipeline"), unsafe_allow_html=True)
...
with k1:
    st.markdown(kpi_card("Analyst Memos", f"{len(sentiment['memos']):,}", "NLP-analyzed"), unsafe_allow_html=True)
with k2:
    vader_dist = sentiment['distributions']['vader']
    pct_pos_vader = vader_dist['Positive'] / sum(vader_dist.values())
    st.markdown(kpi_card("VADER Positive", f"{pct_pos_vader:.1%}", f"{vader_dist['Positive']:,} memos"), unsafe_allow_html=True)
with k3:
    nb_acc = sentiment['naive_bayes']['cv_accuracy']
    st.markdown(kpi_card("NB Accuracy", f"{nb_acc:.1%}", f"5-fold CV"), unsafe_allow_html=True)
with k4:
    h2h = sentiment['head_to_head']
    avg_agreement = np.mean([v for k, v in h2h['agreement_matrix']['VADER'].items() if k != 'VADER'])
    st.markdown(kpi_card("Avg Agreement", f"{avg_agreement:.1%}", "across methods"), unsafe_allow_html=True)
...
# ================================================================
st.markdown(section_header("Sentiment Distribution — Three Methods Compared"), unsafe_allow_html=True)
...
# ================================================================
st.markdown("---")
st.markdown(section_header("Sentiment Score Distributions"), unsafe_allow_html=True)
...
# ================================================================
st.markdown("---")
st.markdown(section_header("Sentiment Breakdown — By Outcome, Sector & Stage"), unsafe_allow_html=True)
...
# ================================================================
st.markdown("---")
st.markdown(section_header("Naive Bayes Classifier — Deep Dive"), unsafe_allow_html=True)
...
# ================================================================
st.markdown("---")
st.markdown(section_header("Head-to-Head: Method Comparison"), unsafe_allow_html=True)
...
# ================================================================
st.markdown("---")
st.markdown(section_header("Analyst Memo Explorer"), unsafe_allow_html=True)
...
# ================================================================
# SECTION H: PRESCRIPTIVE — SENTIMENT AS SCREENING SIGNAL
# ================================================================
st.markdown("---")
st.markdown(section_header("Prescriptive: Sentiment as a Screening Signal"), unsafe_allow_html=True)
...
with col1:
    ...
    for cls in ['Positive', 'Neutral', 'Negative']:
        ...
        if mask_cls:
            sr = np.mean([actual_outcomes[i] for i in mask_cls])
...
# ================================================================
st.markdown("---")
st.markdown(section_header("Analytics Pipeline Position"), unsafe_allow_html=True)
